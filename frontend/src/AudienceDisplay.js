...
function AudienceDisplay() {
  ...
  const renderLives = () => {
    ...
    return (
      <div className="absolute top-5 left-5 flex items-center z-50">
        <span className="text-white font-bold mr-2 drop-shadow-lg text-[clamp(1rem,2.5vw,2rem)]">
          Mystery Guest Life:
        </span>
        <div 
          className="transition-all duration-500 text-[clamp(3rem,8vw,7rem)]"
          style={{
  ...
  const renderGameOver = () => {
    ...
    return (
      <div className="text-center text-white py-16 px-5">
        <div className="font-bold text-red-500 drop-shadow-2xl mb-10 text-[clamp(4rem,15vw,12rem)]">
          💀 GAME OVER 💀
        </div>
        <div className="drop-shadow-lg mb-10 text-[clamp(2rem,7vw,6rem)]">
          {isSecondMatch ? 'Second Match After Life Lost!' : 'Mystery Guest Eliminated!'}
        </div>
        <div className="font-bold text-green-500 drop-shadow-2xl mb-5 text-[clamp(2.5rem,8vw,8rem)]">
          🔒 Final Prize: ₹{finalPrize.toLocaleString()}
        </div>
        <div className="flex justify-center mt-10">
          <div className="filter brightness-50 text-[clamp(5rem,18vw,14rem)]">🖤</div>
        </div>
      </div>
    );
  };
  ...
  const renderMainGame = () => {
    ...
    const renderPrizeLadder = (currentQuestionNumber) => (
      <div className="mb-[clamp(4rem,10vw,12rem)]">
        <div className="bg-gradient-to-r from-[#1976D2] via-[#42A5F5] to-[#1976D2] rounded-full
                        p-[clamp(1.5rem,3vw,3rem)] my-[clamp(2rem,5vw,6rem)] mx-auto
                        max-w-[90vw] w-full border-2 border-[#2196F3] shadow-blue-500/60 shadow-lg relative">
          <div className="font-bold text-white drop-shadow-md text-center text-[clamp(2rem,5vw,4rem)]">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 3xl:grid-cols-7 place-items-center
                        gap-[clamp(1rem,2vw,2.5rem)] mx-auto max-w-[95vw]">
          {prizeTiers.map((tier, index) => {
            ...
            return (
              <div key={index} className={`
                w-[clamp(6rem,12vw,12rem)] h-[clamp(6rem,12vw,12rem)] 
                ${backgroundColor} ${borderColor} border-4 rounded-xl
                flex items-center justify-center
                shadow-xl ${glowColor} relative
                transform rotate-45 m-[clamp(0.5rem,1.5vw,2rem)]`}>
                <div className="transform -rotate-45 font-bold text-white text-center leading-none text-[clamp(1.25rem,3vw,2.5rem)]">
                  {tier.label}
                </div>
                {isLocked && (
                  <div className="absolute -top-1 -right-1 transform -rotate-45 text-[clamp(2rem,4vw,4rem)]">
                    🔒
                  </div>
                )}
                {isMoneyLocked && !isLocked && (
                  <div className="absolute -bottom-1 -right-1 transform -rotate-45 text-[clamp(1.5rem,3.5vw,3.5rem)]">
                    💰
                  </div>
                )}
              </div>
    ...
    return (
      <div className="text-center p-[clamp(2rem,4vw,5rem)] relative w-full max-w-[1600px]">
        ...
        <div className="bg-gradient-to-br from-[#1a1a2e] via-[#16213e] to-[#0f3460] rounded-full
                        py-[clamp(2rem,6vw,6rem)] px-[clamp(2rem,8vw,10rem)] mx-auto 
                        mb-[clamp(4rem,10vw,12rem)] w-full max-w-[90vw]
                        border-4 border-gray-400 shadow-lg shadow-black/30 relative">
          <div className="font-bold text-white drop-shadow-md text-[clamp(3rem,8vw,8rem)]">
        ...
        {currentQuestion.text && currentQuestion.text !== 'No question selected' && currentQuestion.options.length > 0 && (
          <div className="grid grid-cols-1 lg:grid-cols-2
                          gap-[clamp(1rem,2vw,2.5rem)] max-w-[90vw] mx-auto 
            ...
            {currentQuestion.options.map((opt, i) => {
              ...
              // ... your existing color logic remains the same
              ...
              return (
                <div key={i} className={`rounded-full p-[clamp(1rem,2.5vw,2.5rem)]
                  ...
                  <div className="flex items-center gap-[clamp(0.5rem,1.5vw,1.5rem)]">
                    <div className="text-orange-500 font-bold drop-shadow-md w-16 text-left text-[clamp(2rem,5vw,4rem)]">
                      {letter}
                    </div>
                    <div className="text-white font-bold drop-shadow-md flex-1 text-left text-[clamp(2rem,5vw,4rem)]">
                      {opt}
                    </div>
                  </div>
                  
                  {isAnswered && (
                    <div className="absolute top-1/2 -translate-y-1/2 right-8 font-bold text-[clamp(2.5rem,6vw,5rem)]">
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center relative overflow-hidden
      ...
      <h1 className="text-orange-500 uppercase font-bold tracking-wider
                     drop-shadow-lg text-center mb-[clamp(1.5rem,4vw,5rem)] text-[clamp(1.75rem,5vw,4rem)]">
        Judge Me If You Can
      </h1>
      ...
      {gameState ? (
        <div className="w-full flex justify-center">
          {gameState.round === 'main_game' ? (
            ...
            <div className="text-center text-white drop-shadow-md text-[clamp(1.25rem,4vw,3rem)]">
